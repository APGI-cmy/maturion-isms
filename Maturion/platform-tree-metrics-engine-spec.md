...
text
Copy code
DriftScore = normalize(∑ driftEventsWeighted)
Where:
...
41–60 → concerning drift

61–80 → high drift

81–100 → critical drift
...
5.11 CostUsageScore
Represents how cost-effective this node’s operations are in context.
...
Composite score summarizing a node’s state.
...
6.1 Constitutional / Philosophy Nodes (Levels 0–2)
Primary metrics:
...
6.2 Intelligence & Governance Nodes (Levels 3–4)
Key metrics:
...
6.4 Platform Architecture & Modules (Levels 6–8)
Key metrics:
...
RedTimePercentage high + recent incidents → likely 🔴

AmberTimePercentage moderate + driftScore rising → likely 🟡

OverallHealthScore > 80 → 🟢 (if no red/amber present)

ARC override / purple conditions override metric-based colours

Metrics are inputs to status, not the only deciding factor.

9. MATURION’S USE OF METRICS
Maturion uses metrics for:

diagnostics (Phase 3)

prioritisation (“where should we focus?”)

strategy (“is this a one-off bug or systemic issue?”)

governance reasoning (“is autonomy safe here?”)

cost vs safety trade-offs (“is this subsystem too expensive for the risk it covers?”)

Every diagnostics response should be able to cite metrics:

“This node has a high HotspotScore because it has 40% red time and
frequent high-severity incidents in the last 7 days.”
...
“Always green” node → high health score

“Flaky” node (frequent red/green flips) → volatile / high drift

“Chronic red” node → high HotspotScore, low health
