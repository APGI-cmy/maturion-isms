...
- **Guardian** → Output Safety + Policy Compliance  
- **Sentinel** → Behavioural Drift + Anomaly Detection  
- **Arbiter** → Hard Safety Boundaries + Memory + Isolation Enforcement  
...
4. Watchdog → incident → metrics integration  
...
Guardian	🛡️	Safety / output validation issues
Sentinel	👁️	Drift, behavioural anomalies
Arbiter	⚖️	Hard boundaries, memory, isolation
...
“Watchdog Alert: Critical safety or boundary breach”
...
6. NODE DETAIL PANEL — WATCHDOG SECTION
A dedicated “Watchdogs” section appears with:
...
“Reduce autonomy here”

“Trigger calibration”

“Run policy validation suite”

“Open IWMS incident”

“Revoke builder permissions temporarily”
...
info → small faint marker

warn → medium, coloured

error → large, prominent
...
optional: notify Johan’s mobile device
...
isolation_violation → red shield

memory_boundary_violation → world model contamination blocked

crossTenantDataAttempt → tree-wide isolation alert
...
correlations (“drift increased after the last rebuild”)

cause-effect mapping (“failure likely due to cross-module dependency”)

Example output:

“Sentinel reports behavioural drift trending upward for the last 3 hours.
I recommend temporarily reducing autonomy from 3 → 1 and running the
calibration suite.”
...
simulate events → assert UI updates

simulate Arbiter violation → assert autonomy lock + red state
...
Guardian error → amber node

Sentinel error → drift incident

Arbiter error → system-level CRITICAL incident
