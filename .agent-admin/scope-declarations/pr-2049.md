...
RESPONSIBILITY_DOMAIN: Bounded PR-scoped producer-guidance runtime correction plus admin refresh for the GOV-2047 hardening wave, preserving the stable reviewed-head binding while recording refreshed final IAA PASS for the corrected head.

IN_SCOPE:
- Refresh only the bounded PR #2049 admin/control truth required after the producer-guidance runtime correction and current-head assurance refresh.
...
- Preserve the already-committed GOV-2047 governance/controller surfaces exactly as reviewed, except for the bounded current-head producer-guidance runtime correction in the four `.github/scripts` files listed below.
- Keep the existing Foreman PREHANDOVER/session artifacts and the historical IAA wave record in place while accurately preserving the historical token and the refreshed current-head final-PASS record.
