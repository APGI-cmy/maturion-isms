...
RESPONSIBILITY_DOMAIN: Bounded PR-scoped terminal-state safety correction plus admin refresh for the GOV-2047 hardening wave, preserving the stable reviewed-head binding while superseding the earlier final-PASS posture until refreshed current-head assurance is complete.
...
- Keep the existing Foreman PREHANDOVER/session artifacts and the historical IAA wave record in place while accurately preserving historical tokens and superseding the current-head final-PASS posture until refreshed current-head assurance is completed.
...
- FINAL_IAA_STEP: PENDING_REFRESH_CURRENT_HEAD
