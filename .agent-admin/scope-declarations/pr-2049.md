...
RESPONSIBILITY_DOMAIN: Bounded PR-scoped admin refresh for the GOV-2047 hardening wave after HEAD `761340c15fac5e4c1947ac2fb3b910036a15f8f4` corrected producer-guidance runtime behavior, preserving the stable reviewed-head binding while normalizing final-IAA posture back to refreshed-current-head pending.

IN_SCOPE:
- Refresh only the bounded PR #2049 admin/control truth required after HEAD `761340c15fac5e4c1947ac2fb3b910036a15f8f4`.
...
- Keep the existing Foreman PREHANDOVER/session artifacts and the historical IAA wave record in place while accurately withdrawing current-head final-PASS posture until refreshed final IAA is completed.
