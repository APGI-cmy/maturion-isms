# Codex Delegation: api-builder Contract Enhancement Specification
...
The api-builder contract in `modules/mat/04-builder-appointment/builder-contract.md` §2 has been enhanced with Builder-Only Constraints, session memory protocol, and LAS v6.2.0 compliance sections as part of the unified contract document update (Issue #196).
...
1. **Builder-Only Constraint** (§2 of contract):
   - Verify authorized file paths cover all api-builder responsibilities across 7 waves
...
2. **Session Memory Protocol** (§8 of contract):
   - Verify session memory template is complete for api-builder context
...
3. **LAS v6.2.0 Categories** (§0 Governance Binding table):
   ...
   - Category 6: Deliverables & Outputs — §2 scope, test coverage, acceptance criteria
   - Category 7: Prohibitions & Guardrails — §2 Forbidden Actions + Builder-Only Constraint

4. **Merge Gate Support**:
   - api-builder artifacts must support Check 2 (Validate Builder Involvement)
...
- [ ] All 8 LAS v6.2.0 categories satisfied for api-builder
...
*END OF DELEGATION SPECIFICATION — api-builder*
