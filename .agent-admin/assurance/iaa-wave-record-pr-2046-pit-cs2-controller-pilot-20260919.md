...
### 2026-09-19 — PR #2046 @ `c491c984e31bcb3f8e28e8f69a2b9978fa87eb27` — REJECTION-PACKAGE

- **ACR-03 / ACR-08 / OVL-CI-005 — Exact-head active bundle is still stale on the reviewed commit**
  - **Finding:** Final assurance was requested for exact head `c491c984e31bcb3f8e28e8f69a2b9978fa87eb27`, but the active PR-bound truth still binds older heads. The refreshed admin set still declares `fba039505a8c7d6720bf5f452a8eccfa36b959c6` in `.admin/prs/pr-2046.json:11,52`, `.agent-admin/prs/pr-2046/active-state.json:4,13`, `.agent-admin/prs/pr-2046/wave-current-tasks.md:8,16,34,43`, `.agent-admin/scope-declarations/pr-2046.md:9,28,44`, `.agent-admin/assurance/iaa-wave-record-pr-2046-pit-cs2-controller-pilot-20260919.md:13,52,68`, and `.agent-admin/prs/pr-2046/ecap-admin-bundle-20260919.md:12,24,32,44,82,88`; the ordinary route evidence still binds `17b11f6091679b6ec91817e4b30952a7898697e5` in `.agent-admin/prehandover/proof-pr-2046-pit-cs2-controller-pilot-20260919.md:5,71`, `.agent-admin/prehandover/OVL-CI-005-S033-evidence-pr-2046-pit-cs2-controller-pilot-20260919.md:5,105`, and `.agent-workspace/foreman-v2/memory/session-pr-2046-pit-cs2-controller-pilot-20260919.md:13,153`. No active assurance artifact truthfully binds `c491c984e31bcb3f8e28e8f69a2b9978fa87eb27`, so exact-head parity is not established.
  - **Fix required:** Regenerate the entire active PR-2046 bundle so the manifest, active-state file, scope declaration, wave task record, wave record, ECAP bundle, PREHANDOVER proof, S-033 evidence, and Foreman session memory all bind the same reviewed head `c491c984e31bcb3f8e28e8f69a2b9978fa87eb27`, then re-invoke IAA on that committed head.
  - **Classification:** Ceremony / Systemic
  - **Prevention action:** Add a mandatory pre-IAA parity gate that fails if any active PR-bound artifact head SHA differs from `git rev-parse HEAD`.

- **OVL-CI-001 / OVL-CI-004 — PR-scoped pre-brief routing still contradicts governing sources**
  - **Finding:** The implementation still makes the PR-scoped task file authoritative for pre-brief discovery in `.github/workflows/iaa-prebrief-inject.yml:95-97,219`, while the Tier 1 IAA contract still directs Phase 0 to read `.agent-workspace/foreman-v2/personal/wave-current-tasks.md` (`.github/agents/independent-assurance-agent.md:185`) and the canon still declares `.agent-admin/waves/wave-<N>-current-tasks.md` as the wave checklist path (`governance/canon/IAA_PRE_BRIEF_PROTOCOL.md:204-217,286-292,353`). `.agent-admin/control/protocols/IAA_PREFLIGHT_BRIEF_PROTOCOL.md:95-105` adds a PR-scoped-first rule, but that lower-order protocol does not resolve the higher-order contract/canon contradiction.
  - **Fix required:** Align the workflow and supporting protocol with the governing Tier 1 contract/canon, or land the required authorised contract/canon ripple before treating the PR-scoped path as canonical.
