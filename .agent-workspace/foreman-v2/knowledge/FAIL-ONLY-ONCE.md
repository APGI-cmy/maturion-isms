...
| A-019 | ARTIFACT-IMMUTABILITY (Canonical A-19 — 2026-03-04 / PS-B-02 layer-down): Foreman MUST NOT modify any PREHANDOVER proof, session memory file, or other committed evidence artifact after initial commit. Post-commit corrections require a new addendum artifact appended to `.agent-workspace/foreman-v2/memory/`. This is the canonical A-19 rule layer-downed to the ISMS-local registry. The ISMS-local extension (A-028) adds ISMS-specific IAA token file separation and PREHANDOVER pre-population requirements. Canonical ref: maturion-foreman-governance A-19; canonical dedup fix: formerly second canonical A-016, renamed A-019 per S-011. | maturion-foreman-governance A-19 (CS2 auth 2026-03-04); S-011 canonical dedup — REMEDIATED 2026-03-02 |
...
| A-033 | CEREMONY-FILES-IN-SCOPE-DECLARATION (CS2 — 2026-03-06 / PS-B-03): All Foreman ceremony files produced in a wave (wave-current-tasks.md, PREHANDOVER proof, session memory) MUST be listed in SCOPE_DECLARATION.md. **A-031 carve-out for IAA artifacts**: IAA ceremony files (iaa-prebrief-*.md, iaa-token-*.md, iaa-rejection-*.md) are exempt from this scope-declaration requirement because they ARE the Pre-Brief or its outcome artifact — requiring them to be pre-declared in SCOPE_DECLARATION would create a circular dependency with A-031. Session memory and PREHANDOVER proof are NOT exempt and MUST appear in SCOPE_DECLARATION. Violation class: INC-SCOPE-CEREMONY-001. | session-postfcwt-prodfails (PS-B-03 v4.2.0) |
| A-034 | CANON-INVENTORY-UPDATE-MANDATORY (CS2 — PS-B-04): Any wave that modifies, adds, or removes a file that is tracked in `governance/CANON_INVENTORY.json` MUST include an update to `governance/CANON_INVENTORY.json` in the SAME wave (same PR). A wave that modifies a canonical governance file without updating CANON_INVENTORY fails the OPOJD gate (A-004). The Foreman MUST verify at QP evaluation that any canonical file modification is accompanied by a CANON_INVENTORY.json update in the diff. If the file is not yet in CANON_INVENTORY, it must be added in the same wave. Violation class: INC-CANON-INVENTORY-STALE-001. | session-wave-disable-auto (PS-B-04 v4.2.0) |
| A-035 | DELEGATION-ISSUE-REQUIRED (CS2 — 2026-03-08 / PS-B-05): Every builder delegation MUST have a corresponding GitHub issue created and linked in the session memory BEFORE the builder begins work. A delegation noted without a GitHub issue number is an incomplete commission. The Foreman MUST verify each row in the session memory "Agents Delegated To" table has a corresponding `maturion-isms#NNN` issue URL before writing the PREHANDOVER proof. A delegation recorded as "PENDING issue" or without an issue URL is a Phase 4 OPOJD gate failure. Codifies S-025 (DELEGATION-ISSUE-REQUIRED — OPEN since INC-OPOJD-W15R-QA-001 2026-03-08). Violation class: INC-DELEGATION-NOISSUE-001. | session-wave-disable-auto-R2 (PS-B-05 v4.2.0); S-025 codified |
| A-036 | NO-COMPLEXITY-THRESHOLD-EXEMPTION (CS2 — 2026-03-10): There is NO minimum file count, line count, or complexity threshold below which the mandatory pre-wave governance sequence (Phase 1 PREFLIGHT → wave-current-tasks.md → IAA Pre-Brief → builder delegation) may be skipped. A 1-line fix, a 1-file CI workflow change, and a 200-file wave all require identical governance sequence. The Foreman's cognitive shortcut "small explicit fix = safe to implement directly" is a persistent violation class (seventh occurrence: INC-WCA-PREBRIEF-IMPL-001). Any PR touching `.github/workflows/`, `apps/`, `modules/`, `supabase/`, or `packages/` paths authored by foreman-v2-agent is a POLC violation regardless of the change scope. Violation class: INC-BOOTSTRAP-IMPL-001. | CS2 re-alignment directive — maturion-isms PR copilot/update-agent-contract-audit-workflow (2026-03-10); INC-WCA-PREBRIEF-IMPL-001 |
...
| A-044 | FOREMAN-OWNED-PREREQUISITE-NON-STOP-AND-EXACT-HEAD-LOOP-PROHIBITION (CS2 — 2026-09-19 / issue #2047): (1) A Foreman-owned ordinary prerequisite — missing/stale tooling, an evidence-format defect, routine config, or an administrative artifact that Foreman itself may lawfully correct — is NEVER a valid HALT or escalation. Foreman MUST self-remediate per `foreman-tier2-operating-protocol.md` §9a before any STOP_AND_FIX or CS2 escalation is issued for such a defect; a blocker report skipping self-remediation for an ordinary Foreman-owned prerequisite is itself a governance defect. Escalation to CS2 is valid only for a proven protected-authority or external boundary (CS2-only canon amendment, destructive/cost decision, unresolved business authority, or genuine external dependency), named exactly. (2) Foreman and IAA MUST NOT create a repetitive evidence-only commit whose sole purpose is to make an artifact describe its own newly changed HEAD (the "exact-current-HEAD self-referential evidence loop"). Assurance binds to the stable reviewed submission head already on record, or to an independent external attestation; a commit produced only to refresh a HEAD-reference inside its own evidence artifact does not satisfy any evidence requirement and is a protocol violation. Violation class: INC-FOREMAN-FALSE-STOP-001 / INC-EXACT-HEAD-LOOP-001 (preventive lock-in, issue #2047, PR #2049; precedent pattern: `.agent-workspace/CodexAdvisor-agent/knowledge/continuous-improvement-protocol.md` §1–§2, CS2-authorised via PR #2048). | CS2 authorization 2026-09-19 (issue #2047; PR #2049 GOVERNANCE-2047-FOREMAN-CONVERGENCE-20260919) |
...
> **Corrective action completion markers (PS-B-06 — v4.2.0)**: Corrective action items within incident entries use `[ ]` (pending/incomplete) and `[x]` (completed) markers. When reviewing an incident, check that all `[x]`-marked items are verifiably complete. Any item still showing `[ ]` in a `REMEDIATED` incident must be escalated to CS2.
...
5. A-036 (NO-COMPLEXITY-THRESHOLD-EXEMPTION) locked in FAIL-ONLY-ONCE.md.
...
**New A-rule candidate (now A-038): COPILOT-BUILDER-ROLE-LABEL-BYPASS-PROHIBITION: The `copilot-builder-role` PR label that bypasses `polc-boundary-gate.yml` foreman/session-memory checks MUST NOT be applied to PRs where the foreman-v2-agent is the session author. The label is intended for pure builder sessions (api-builder, ui-builder, etc.) where Foreman is not the executing agent. When foreman-v2-agent is the session author, the full POLC boundary gate including session-memory-check must run regardless of any PR label. Escalate to CS2 for A-038 lock-in and polc-boundary-gate.yml amendment.
### INC-CI-LIVENESS-FIX-001 — Foreman IAA Pre-Brief Skipped and IAA Token Not Obtained: CI Liveness Workflow Fix (2026-03-18)
**Date**: 2026-03-18
**Severity**: MAJOR
**Status**: REMEDIATED — CS2 re-alignment directive acknowledged (2026-03-18); RCA completed; lessons learned file committed; A-037 formally locked in; S-035 improvement suggestion added; FAIL-ONLY-ONCE updated to v4.0.0; learning loop activated.
...
4. A-037 CI-FIX-NO-EXEMPTION formally locked in as mandatory rule (no longer a candidate). ✅
...
| Version | Date | Change |
|---------|------|--------|
| 4.8.0 | 2026-09-19 | A-044 FOREMAN-OWNED-PREREQUISITE-NON-STOP-AND-EXACT-HEAD-LOOP-PROHIBITION locked in as mandatory permanent rule — Foreman-owned ordinary prerequisites (tooling/evidence/config/admin defects) must be self-remediated and are never a valid HALT or escalation; escalation is valid only for a proven protected/external boundary; repetitive evidence-only commits to refresh an artifact's own current-HEAD reference are prohibited, assurance binds to a stable reviewed head or independent external attestation instead. `foreman-tier2-operating-protocol.md` §9a (blocker classification/remediation ladder) added as the Tier 2 detail home for this rule. Wave: GOVERNANCE-2047-FOREMAN-CONVERGENCE-20260919 (issue #2047, PR #2049). |
...
| 4.6.0 | 2026-05-06 | INC-GATE-HANDOVER-FAIL-001 registered: Foreman handed over PR #1541 (wave: pit-stage1-cs2-approval-stage2-initiation) with 4 failing CI gates: preflight/mmm-pr-admin (CHECK 9 SCOPE-TO-DIFF — 3 Phase 4 files missing from .admin/pr.json scope array), preflight/iaa-prebrief-existence (wave-current-tasks.md missing iaa_wave_record_path: bare field), preflight/scope-declaration-parity (pr-1541.md used wrong format — 7 required bare-key fields absent: ISSUE, BRANCH, OWNER, FILES_CHANGED, OUT_OF_SCOPE, DATE_UTC, PR_NUMBER as structured keys), preflight/evidence-exactness (secondary to scope declaration format failure). CS2/Johan Ras corrective directive issued via PR #1541 comment 4387593448: "This ecosystem does not allow for the handing over of failing gates. Handing over a failing gate is a catastrophic failure." Root cause: (1) gate simulation run locally used no origin/main ref (CHECK 9 skipped in sandbox); (2) scope declaration template from prior PR not checked against current CI enforcement script; (3) wave-current-tasks.md format used Markdown bold `**IAA Wave Record**:` instead of bare `iaa_wave_record_path:` key. REMEDIATED in follow-on commit (same PR). S-041 CI-GATE-DRY-RUN-WITH-DIFF-MANDATORY: Before any report_progress/handover, Foreman MUST verify gate parity by fetching origin/main into sandbox and running validate-simple-pr-admin.sh + enforce-scope-declaration-policy.sh with origin/main available so CHECK 9 and evidence-exactness COUNT-MISMATCH actually execute. Candidate for A-043. |
| 4.5.0 | 2026-04-22 | A-040 TEMPORAL-AUDIT-AT-QP and A-041 EVIDENCE-TYPE-CLASSIFICATION-AT-QP locked in — governance hardening following PR #1444 review miss: Foreman QP must check for future-dated factual claims (A-040) and evidence-type mismatch on deployment/CDV items (A-041) at every builder handover evaluation. Canon: `governance/canon/TEMPORAL_AND_EVIDENCE_INTEGRITY_CANON.md`. Issue: maturion-isms#1445. |
...
| 4.0.0 | 2026-03-18 | INC-CI-LIVENESS-FIX-001 registered (Foreman IAA Pre-Brief skipped and IAA token not obtained before handover: CI liveness workflow fix session on PR copilot/fix-ci-update-liveness-workflow; eleventh occurrence of A-031 + A-014 violation class; CS2 re-alignment issued 2026-03-18; RCA completed; lessons learned file committed; A-037 CI-FIX-NO-EXEMPTION formally locked in as mandatory rule; S-035 CI-PREBRIEF-GATE-WORKFLOW-PATH-EXTENSION added; version bumped to 4.0.0 — major version due to formal lock-in of A-037 after candidate status at v3.8.0) |
...
| 3.8.0 | 2026-03-12 | INC-CI-GATEWAY-FIX-001 registered (Foreman IAA Pre-Brief skipped and IAA token not obtained before handover: CI gateway fix session on PR copilot/fix-ci-gateway-failure; ninth occurrence of A-031 + A-014 violation class; CS2 re-alignment issued 2026-03-12; retroactive IAA Pre-Brief and token obtained; learning loop activated); A-037 candidate CI-FIX-NO-EXEMPTION documented |
