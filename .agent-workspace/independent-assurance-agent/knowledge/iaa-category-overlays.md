...
| OVL-SAA-004 | Was product-delivery evidence required or correctly suppressed? | If no runtime product code in `apps/`/`modules/`/`packages/` and no explicit product-delivery claim in PR body: product-delivery evidence was correctly suppressed. If product code or explicit claim present: product-delivery evidence was required. | FAIL: state which evidence model was wrongly applied |
