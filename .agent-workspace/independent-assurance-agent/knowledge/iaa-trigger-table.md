...
8. Does PR modify pre-build stage governance artifacts (PRE_BUILD_STAGE_MODEL, module.manifest.json, BUILD_PROGRESS_TRACKER.md, module lifecycle stages for any of the 12 canonical stages)?
   → YES: Category = PRE_BUILD_STAGE_MODEL. PRE_BUILD_GATES overlay (OVL-PBG-001–OVL-PBG-019) applies. Stage-readiness view required at Pre-Brief. IAA = MANDATORY.

9. Is this a product-facing BUILD/T2 PR, or does the PR claim functional/full/partial delivery or handover readiness, or include `.functional-delivery/pr-<PR>.md`?
   → YES: Category = PRODUCT_BUILD_ASSURANCE. IAA = MANDATORY. Load PRODUCT_BUILD_ASSURANCE_STANDARD.md before verdict.

10. Does PR modify cross-app component governance (MANDATORY_CROSS_APP_COMPONENTS, packages/ mandatory components, cross-app compliance claims)?
   → YES: Category = MANDATORY_CROSS_APP_COMPONENTS. IAA = MANDATORY.

11. Does PR contain ONLY retrospective audit artifacts (session memory files, PREHANDOVER proofs, IAA token files, parking station log entries, PREHANDOVER addendum files)?
...
12. Is the PR clearly and unambiguously doc-only, parking-station, or admin?
...
| 2.6.0 | 2026-05-10 | PRODUCT_BUILD_ASSURANCE trigger category added (governing issue #1596; incident calibration PR #1590): product-facing BUILD/T2 or functional-delivery-claim PRs now require mandatory loading of PRODUCT_BUILD_ASSURANCE_STANDARD.md before verdict; decision flow updated with explicit product-build classification step. |
| 2.7.0 | 2026-05-14 | PRE_BUILD_STAGE_MODEL downstream sync for issue #1635 hardening: updated PRE_BUILD_GATES range references from OVL-PBG-001 through OVL-PBG-017 to OVL-PBG-001 through OVL-PBG-019 in both trigger row and classification step 8 to satisfy OVL-PBG-ADM-002. |
