...
|------|---------|---------|--------|
| `index.md` (this file) | Knowledge entry point and version reference | 4.3.0 | PRESENT |
...
| `iaa-trigger-table.md` | PR category classification table — when IAA activates and when it is exempt | 2.7.0 | PRE_BUILD_STAGE_MODEL downstream sync completed: PRE_BUILD_GATES overlay references now align to OVL-PBG-001 through OVL-PBG-019 in trigger row and classification flow (OVL-PBG-ADM-002; issue #1635) |
| `iaa-category-overlays.md` | Per-category additional checks (AGENT_CONTRACT, CANON_GOVERNANCE, CI_WORKFLOW, AAWP_MAT, KNOWLEDGE_GOVERNANCE, PRE_BRIEF_ASSURANCE, PRE_BUILD_GATES, GOVERNANCE_EVIDENCE, SIMPLIFIED_ADMIN_ASSURANCE, STRICT_MERGE_POSTURE) | 4.6.0 | PRE_BUILD traceability hardening added: OVL-PBG-018 (`IAA_PRE_BUILD_TRACEABILITY-001` matrix required before pre-build PASS/readiness), OVL-PBG-019 (PR #1630 regression calibration coverage), explicit unsupported-claim rejection posture `REJECTED_BACK_TO_PRODUCER`; governing issue #1635 |
...
| `PRODUCT_BUILD_ASSURANCE_STANDARD.md` | Product-facing BUILD/T2 functional-delivery assurance standard (promised journey, CTA capability map, schema contract, async/pending visibility, split verdict discipline, rejection package shape) | 1.1.0 | ACTIVE — MANDATORY load for product-facing BUILD/T2/UI/backend/API delivery PRs and PRs claiming functional/full/partial/handover delivery (governing issue #1596; incident calibration PR #1590); includes no-current-head-drift re-invocation rule and explicit non-mergeable partial default wording |
...
- `governance/canon/AGENT_HANDOVER_AUTOMATION.md` v1.1.4
...
## Product-Build Mandatory Loading Rule

For product-facing BUILD/T2 assurance, IAA MUST load and apply:

- `PRODUCT_BUILD_ASSURANCE_STANDARD.md`
- `iaa-category-overlays.md` (BUILD_DELIVERABLE + GOVERNANCE_EVIDENCE where applicable)
- `FUNCTIONAL-BEHAVIOUR-REGISTRY.md`
- `niggle-pattern-library.md`

Product-facing BUILD/T2 includes UI/backend/API delivery PRs and PRs claiming functional/full/partial delivery or handover readiness.
...
| A-036 | Future-Dated Factual Claims Are Blockers — for GOVERNANCE_EVIDENCE/GOVERNANCE_AUDIT PRs: any completion claim with a future-dated timestamp is REJECTION-PACKAGE. Canon: TEMPORAL_AND_EVIDENCE_INTEGRITY_CANON.md T-001/T-002 | ACTIVE |
| A-037 | Evidence-Type Discipline — LIVE_RUNTIME/LIVE_E2E items cannot be satisfied by STATIC_CODE/CI_TEST/CONFIG evidence. Code-merge evidence does not prove live deployment. Canon: TEMPORAL_AND_EVIDENCE_INTEGRITY_CANON.md E-001/E-002/E-003 | ACTIVE |
| A-038 | §7.x-OVL-PBG coupling — any §7.x addition to PRE_BUILD_STAGE_MODEL_CANON.md requires simultaneous OVL-PBG-NNN addition to iaa-category-overlays.md and §7.x reference update to iaa-trigger-table.md PRE_BUILD_STAGE_MODEL supporting controls; wave mmm-deploy-strategy-oversight-20260426 | ACTIVE |
| A-039 | Agent Claims Are Not Evidence — Acceptance-Criteria Matrix required before PASS; every non-waived governing-issue acceptance criterion must map to a hard artifact; ACR-22 fires if absent/incomplete; maturion-isms#1492 | ACTIVE |
| A-040 | Evidence-Type Downgrade Prohibition — runtime evidence (CI_TEST/LIVE_RUNTIME/LIVE_E2E) cannot be substituted with STATIC_CODE/pattern-parity/agent-attestation without CS2 waiver; gate_triggered: false accepted only when IAA independently verifies no gate-triggering paths changed; ACR-23 fires on downgrade; maturion-isms#1492 | ACTIVE |
| A-041 | Diff-First Classification — IAA must independently compute actual changed-file set from diff; declared scope/category is a claim; actual diff wins on contradiction; ACR-25 fires on classification mismatch; maturion-isms#1492 | ACTIVE |
| A-042 | Independent Risk Challenge mandatory before PASS token — five-question challenge must be completed and recorded; affirmative duty to identify risks not named by any checklist item; ACR-26 fires if absent or incomplete; maturion-isms#1492 | ACTIVE |
...
IAA operates with a single objective: binary verdict — merge permitted or merge blocked.
- **ASSURANCE-TOKEN (PASS)**: all checks PASS, all acceptance criteria verified with hard evidence, Independent Risk Challenge complete → merge permitted (subject to CS2 approval)
- **REJECTION-PACKAGE**: one or more checks FAIL → merge blocked, all failures cited with fix required
- **BLOCKED_PENDING_RUNTIME_EVIDENCE**: implementation plausible but required CI_TEST/LIVE_RUNTIME/LIVE_E2E evidence missing → merge blocked until evidence available or CS2 waiver committed
- **BLOCKED_PENDING_BUILD_CORRECTNESS**: build architecture or functionality compliance not proven → merge blocked
- **PASS_WITH_CS2_WAIVER**: missing evidence explicitly waived by CS2 in a committed artifact → merge permitted under waiver
- **INVALID_PRIOR_TOKEN**: prior token invalidated by new evidence, contradiction, or diff change → merge blocked; prior token no longer valid
...
| 3.8.0 | 2026-04-28 | Evidence-first IAA assurance restoration wave (maturion-isms#1492): INDEPENDENT_ASSURANCE_AGENT_CANON.md → v1.13.0 (§Evidence-First Assurance Mandate — 7 rules; expanded verdict taxonomy; ACR-22 through ACR-26; updated output spec); FAIL-ONLY-ONCE.md → v3.0.0 (A-039 through A-042 — agent claims non-evidence, evidence-type downgrade prohibition, diff-first classification, independent risk challenge); iaa-core-invariants-checklist.md → v4.1.0 (CORE-026/CORE-027 added; IAA-retained invariants 2→4); iaa-category-overlays.md → v4.3.0 (OVL-GE-004 added — acceptance-criteria coverage check); governance/templates updated: iaa-wave-record.template.md → v1.2.0 (§3.2–3.4 added), PREHANDOVER_PROOF_TEMPLATE.md → v3.4 (§Acceptance-Criteria Matrix section + certification item 15), PREHANDOVER.template.md → v1.6.0; CANON_INVENTORY.json hashes updated for all modified canon files |
| 4.0.0 | 2026-05-10 | Product-build assurance hardening (governing issue #1596; incident calibration PR #1590): added PRODUCT_BUILD_ASSURANCE_STANDARD.md v1.0.0 as mandatory Tier 2 load for product-facing BUILD/T2 and functional-delivery-claim PRs; added explicit Product-Build Mandatory Loading Rule section. |
| 4.2.0 | 2026-05-14 | issue #1635 minimal-scope update: iaa-category-overlays.md → v4.6.0 with PRE_BUILD traceability hardening (IAA_PRE_BUILD_TRACEABILITY-001 independent source-to-package matrix, mandatory pre-build matrix checks, explicit `REJECTED_BACK_TO_PRODUCER` posture for unsupported PASS/readiness claims, and PR #1630 regression coverage requirements); governance/templates/iaa-wave-record.template.md → v1.5.0 with visible pre-build traceability matrix fields. |
| 4.3.0 | 2026-05-14 | issue #1635 downstream sync completion: iaa-trigger-table.md → v2.7.0 updated PRE_BUILD_GATES references from OVL-PBG-001 through OVL-PBG-017 to OVL-PBG-001 through OVL-PBG-019 in PRE_BUILD_STAGE_MODEL trigger row and classification step 8 (OVL-PBG-ADM-002 compliance). |
