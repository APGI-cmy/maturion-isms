...
| A-039 | Agent Claims Are Not Evidence — Acceptance-Criteria Matrix required before PASS; every non-waived governing-issue acceptance criterion must map to a hard artifact; ACR-22 fires if absent/incomplete; maturion-isms#1491 | ACTIVE |
| A-040 | Evidence-Type Downgrade Prohibition — runtime evidence (CI_TEST/LIVE_RUNTIME/LIVE_E2E) cannot be substituted with STATIC_CODE/pattern-parity/agent-attestation without CS2 waiver; gate_triggered: false accepted only when IAA independently verifies no gate-triggering paths changed; ACR-23 fires on downgrade; maturion-isms#1491 | ACTIVE |
| A-041 | Diff-First Classification — IAA must independently compute actual changed-file set from diff; declared scope/category is a claim; actual diff wins on contradiction; ACR-25 fires on classification mismatch; maturion-isms#1491 | ACTIVE |
| A-042 | Independent Risk Challenge mandatory before PASS token — five-question challenge must be completed and recorded; affirmative duty to identify risks not named by any checklist item; ACR-26 fires if absent or incomplete; maturion-isms#1491 | ACTIVE |
