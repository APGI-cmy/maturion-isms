...
| **Gate-changing PR without current-PR gate-test evidence** | PR modifies a CI gate, workflow check, or merge gate logic without evidence that the gate was tested against the current PR's HEAD | Require gate-test evidence (CI log or dry-run output) against current HEAD before MERGE_READY |
