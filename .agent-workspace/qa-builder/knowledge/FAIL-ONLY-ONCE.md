# FAIL-ONLY-ONCE Registry — qa-builder
...
This registry records governance failures that qa-builder must never repeat. Each entry captures a
root cause, the permanent rule that prevents recurrence, and the incident reference.

qa-builder loads this file in Phase 1 on every session start and applies all rules.
...
The BOOTSTRAP DIRECTIVE is non-negotiable. If qa-builder reads any repo file before completing
