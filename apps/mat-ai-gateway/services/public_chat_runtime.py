...
class PublicChatService:
    ...
    def answer(
        ...
        if decision.permitted and decision.reservation_id:
            ...
            try:
                ...
                raise
            self._reserve_and_reconcile_tokens(decision.reservation_id, usage["total_tokens"])
    ...
    def _paid_call_permitted(
        ...
        empty = SpendDecision(False, "paid_calls_disabled", None, budget_day, 0, 0, "closed", "none")
        ...
        if route == "maturion_only":
            return SpendDecision(False, "restricted_request_static_response", None, budget_day, 0, 0, "closed", "none")
        if not self._paid_calls_enabled():
            return empty
        return self._reserve_paid_call(context)

    def _reserve_paid_call(self, context: dict[str, Any] | None = None) -> SpendDecision:
        if os.environ.get("PYTEST_CURRENT_TEST") and not isinstance(
            self._usage_store, InMemoryDurableSpendStore
        ):
    ...
    def _reserve_and_reconcile_tokens(self, reservation_id: str, actual_tokens: int) -> None:
        try:
            self._usage_store.reconcile_success(reservation_id, actual_tokens)
        except RuntimeError as exc:
            raise RuntimeError("durable spend reconciliation unavailable") from exc

    def _reconcile_failed_provider_call(self, reservation_id: str) -> None:
        try:
    ...
    def _raw_client_identifier(context: dict[str, Any]) -> str:
        supplied = str(context.get("client_request_id") or "anonymous")[:200]
        test_scope = os.environ.get("PYTEST_CURRENT_TEST", "")[:200]
    ...
    def _complete(self, messages: list[dict[str, str]]) -> tuple[str, dict[str, int]]:
        ...
        return content.strip() or "Maturion could not generate a response just now.", usage
    ...
    @staticmethod
    def _resolve_non_negative_int(name: str, default: int) -> int:
        try:
    ...
    def _build_messages(
        ...
        public_context = f"Public website context data: source=apw-public-website; page={page}"
        messages = [
            {
                "role": "system",
                "content": (
                    "You are Maturion, APGI's public website AI guidance layer. "
                    "Help visitors understand APGI, loss prevention, organisational maturity, "
                    "APGI Hub, training, risk, assurance, implementation pathways, and next steps. "
                    "Keep responses concise, practical, and public-safe. Do not claim access to "
                    "private ISMS workspaces, client records, or live customer data. If a question "
                    "needs private context, invite the visitor to contact APGI or use the governed "
        ...
        if apw_result and apw_result.get("draft"):
            messages.append({"role": "user", "content": self._apw_draft_context(apw_result)})
    ...
    @staticmethod
    def _is_public_apw_question(message: str, page: str, context: dict[str, Any]) -> bool:
        lower = f"{message} {page} {context.get('source', '')}".lower()
        public_terms = ("apw", "apgi", "onboarding", "public website", "demo", "hub", "maturion")
        return any(term in lower for term in public_terms) and not requires_private_context(message)
    ...
    def _public_apw_context_envelope() -> dict[str, Any]:
        return {
            ...
            "user": {"authenticated": False, "id": None, "organisation_id": None, "tenant_id": None},
            "knowledge_scope": {
    ...
    def _static_response(apw_result: dict[str, Any]) -> str:
        ...
        if route == "maturion_only":
            return (
                "Maturion cannot access or disclose private, client, customer, account, record, "
                "credential, token, secret, or internal configuration information. Please contact "
                "APGI directly or use the governed APGI Hub pathway."
            )
