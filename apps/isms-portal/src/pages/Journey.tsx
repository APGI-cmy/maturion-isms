...
import { Target, ChevronRight, ArrowLeft, Shield, Settings, Database, Search, Crosshair, AlertTriangle, MonitorSpeaker, HeartHandshake, Key, CalendarCheck, LifeBuoy, Wrench, Info, Ban, Clock, CheckCircle, Play, Zap as ZapIcon, BarChart2, Cog } from 'lucide-react';
...
const Journey = () => {
  ...
  const [, setHoveredDomain] = useState<number | null>(null);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50" style={{ overflow: 'visible' }}>
                    ...
                    <div className="space-y-4 mt-4">
                      <div className="space-y-2">
                        ...
                        <p className="text-sm text-muted-foreground leading-relaxed">
                          {(domain.levelContent as Record<number, {oneLiner: string; paragraph: string}>)[selectedLevel]?.paragraph || domain.description}
                        </p>
                      </div>
      ...
      >
        <p className="text-sm text-gray-700">
          {(MATURITY_DOMAINS.find(d => d.position === "top")?.levelContent as Record<number, {oneLiner: string; paragraph: string}> | undefined)?.[selectedLevel]?.oneLiner || 
      ...
      {MATURITY_DOMAINS.filter(d => d.position.startsWith("middle")).map((domain, index) => (
        ...
        >
          <p className="text-sm text-gray-700">
            {(domain.levelContent as Record<number, {oneLiner: string; paragraph: string}>)[selectedLevel]?.oneLiner || domain.description}
          </p>
        </TooltipPortal>
      ))}
      ...
      >
        <p className="text-sm text-gray-700">
          {(MATURITY_DOMAINS.find(d => d.position === "bottom")?.levelContent as Record<number, {oneLiner: string; paragraph: string}> | undefined)?.[selectedLevel]?.oneLiner || 
