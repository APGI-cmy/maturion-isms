...
**LUIEP Gate (introduced in wave `mmm-ui-evidence-pack-hardening-20260430`, issue #1523)**: The enforcement canon for the Live UI Evidence Pack requirement is `governance/canon/MMM_UI_EVIDENCE_PACK_GATE.md` v1.0.0 (Rules U-001 through U-005). The **governance requirement is immediately effective upon merge of this wave** — any MMM PREHANDOVER proof claiming L2 or L3 completion after this wave merges MUST be accompanied by a committed LUIEP artifact. The CI gate `preflight/mmm-ui-evidence-pack-gate` (script: `.github/scripts/validate-mmm-ui-evidence-pack.sh`) provides programmatic enforcement; the CI script will be implemented in a subsequent builder wave. Until the CI script is active, Foreman QP and IAA are responsible for manual verification per Rules U-001 through U-005. FAIL-ONLY-ONCE cross-ref: A-043.
