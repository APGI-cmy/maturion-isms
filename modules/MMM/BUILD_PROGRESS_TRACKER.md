...
- [ ] **Live UI Evidence Pack (LUIEP) committed and validated per `governance/canon/MMM_UI_EVIDENCE_PACK_GATE.md`** — A structured LUIEP artifact (template: `modules/MMM/12-phase4-ecap/mmm-ui-evidence-pack-template.md`) containing CS2 sign-off, live deployment URL confirmation, at least one screenshot, and at least one confirmed end-to-end workflow MUST be committed to the repository before any PREHANDOVER proof may claim L2 (Deployment Commissioned) or L3 (Operationally Closed) completion. Checklist: `governance/checklists/mmm-ui-evidence-pack-checklist.md`. Violation class: `INC-MMM-LUIEP-MISSING-001`.
...
**LUIEP Gate (introduced in wave `mmm-ui-evidence-pack-hardening-20260430`, issue #1523)**: The enforcement canon for the Live UI Evidence Pack requirement is `governance/canon/MMM_UI_EVIDENCE_PACK_GATE.md` v1.0.0 (Rules U-001 through U-005). The **governance requirement is immediately effective upon merge of this wave** — any MMM PREHANDOVER proof claiming L2 or L3 completion after this wave merges MUST be accompanied by a committed LUIEP artifact. The CI gate `preflight/mmm-ui-evidence-pack-gate` (script: `.github/scripts/validate-mmm-ui-evidence-pack.sh`) provides programmatic enforcement after merge. Foreman QP and IAA remain responsible for verifying that submitted evidence substantively satisfies Rules U-001 through U-005 and for rejecting any PREHANDOVER claim that attempts to overstate completion status. FAIL-ONLY-ONCE cross-ref: A-043.
