/**
 * Wave 9.6 RED Gate — Risk Management AIMC Wiring Invariants
 * Tests: RISK-AIMC-T-001 to RISK-AIMC-T-006
 *
 * QA-to-Red mandate: ALL tests in this file MUST fail (RED) before implementation.
 * These tests define acceptance criteria for the Wave 9.6 Risk Management AIMC wiring build.
 * They will pass (GREEN) only after api-builder/integration-builder deliver:
 *   - modules/risk-management/src/services/aimc-wiring.ts
 *
 ...
 *   RISK-AIMC-T-001 GREEN — PersonaLoader path corrected to plural 'personas' (regression guard)
 *   RISK-AIMC-T-002 RED — modules/risk-management/src/services/aimc-wiring.ts does not exist
 *   RISK-AIMC-T-003 RED — aimc-wiring.ts does not exist (cannot read content)
 *   RISK-AIMC-T-004 RED — aimc-wiring.ts does not exist (cannot read content)
 *   RISK-AIMC-T-005 RED — aimc-wiring.ts does not exist (cannot read content)
 *   RISK-AIMC-T-006 RED — modules/risk-management/src does not exist (sanity check fails)
 */
...
const RISK_SRC_DIR = resolveFromRoot('modules/risk-management/src');

// ─────────────────────────────────────────────────────────────────────────────
// Test suite
// ─────────────────────────────────────────────────────────────────────────────

describe('Wave 9.6 RED Gate — Risk Management AIMC Wiring Invariants', () => {
  // ──────────────────────────────────────────────────────────────────────────
  // RISK-AIMC-T-001
  // ──────────────────────────────────────────────────────────────────────────
  it(
    'RISK-AIMC-T-001: PersonaLoader.load("risk-advisor") returns non-empty Markdown string',
    async () => {
      /**
       * Architecture mandate (ARCH-FREEZE-WAVE9-TRACK-C-20260226 §3.1):
       *   A persona file must exist at packages/ai-centre/src/agents/risk-advisor.md
       *   and must be loadable via PersonaLoader.load().
       *
       * Acceptance criteria:
       *   1. PersonaLoader can be imported (module exists)
       *   2. PersonaLoader.load('risk-advisor') resolves to a non-empty string
       ...
       *   risk-advisor.md exists; this test guards that PersonaLoader continues
       *   to load the persona correctly after Wave 9.6 implementation.
       *
       * Note: The persona file risk-advisor.md exists; this test is a regression
  ...
  // ──────────────────────────────────────────────────────────────────────────
  it(
    'RISK-AIMC-T-002: Risk Management AIMC wiring service exists at expected module path',
    () => {
      /**
       * Architecture mandate (ARCH-FREEZE-WAVE9-TRACK-C-20260226 §3.1):
       *   The Risk Management AIMC wiring service MUST be located at:
       *     modules/risk-management/src/services/aimc-wiring.ts
       *
       * Acceptance criteria:
       *   File exists at the canonical path defined in the architecture freeze.
       *
       * Status: RED — modules/risk-management/src/services/aimc-wiring.ts does not exist.
       *   Will turn GREEN when api-builder/integration-builder delivers Wave 9.6 impl.
       */
      expect(
        fs.existsSync(WIRING_SERVICE_PATH),
        `Risk Management AIMC wiring service MUST exist at: ${WIRING_SERVICE_PATH}\n` +
  ...
  // ──────────────────────────────────────────────────────────────────────────
  it(
    'RISK-AIMC-T-003: Risk Management wiring service calls POST /api/ai/request',
    () => {
      /**
  ...
  // ──────────────────────────────────────────────────────────────────────────
  it(
    "RISK-AIMC-T-004: Risk Management wiring service uses `capability: 'advisory'` in request payload",
    () => {
      /**
       * Architecture mandate (ARCH-FREEZE-WAVE9-TRACK-C-20260226 §3.1):
  ...
  // ──────────────────────────────────────────────────────────────────────────
  it(
    "RISK-AIMC-T-005: Risk Management wiring service uses `agent: 'risk-advisor'` in request payload",
    () => {
      /**
       * Architecture mandate (ARCH-FREEZE-WAVE9-TRACK-C-20260226 §3.1):
       *   The wiring service MUST pass `agent: 'risk-advisor'` in the AIMC request.
       *   This ensures AIMC selects the correct persona for Risk Management advisory responses.
       *
       * Acceptance criteria:
       *   aimc-wiring.ts source code contains `agent: 'risk-advisor'`
      ...
      expect(
        content,
        "aimc-wiring.ts MUST include `agent: 'risk-advisor'` (or double-quoted variant) " +
          'in the AIMC gateway request payload. ' +
          "This routes the request to the 'risk-advisor' persona registered in AIMC.",
      ).toMatch(/agent\s*:\s*['"]risk-advisor['"]/);
    },
  );
  ...
  // ──────────────────────────────────────────────────────────────────────────
  it(
    'RISK-AIMC-T-006: No direct OpenAI/Anthropic/provider SDK imports exist in risk-management module source',
    () => {
      /**
       * Architecture mandate (ARCH-FREEZE-WAVE9-TRACK-C-20260226 §2.2):
       *   Risk Management module source MUST NOT import provider SDKs directly.
       ...
       *
       * Acceptance criteria:
       *   1. modules/risk-management/src directory is non-empty (sanity check)
       *   2. Zero source files contain forbidden provider SDK imports
       *
       * Status: RED — modules/risk-management/src does not exist yet (sanity check will fail).
       *   Will turn GREEN after Wave 9.6 delivery (src exists with no provider imports).
       */
      const sourceFiles = collectSourceFiles(RISK_SRC_DIR);

      expect(
        sourceFiles.length,
        `modules/risk-management/src must be non-empty (sanity check). ` +
          `Path checked: ${RISK_SRC_DIR}\n` +
      ...
      expect(
        violatingFiles,
        'No Risk Management module source files may import provider SDKs directly. ' +
