...
11. **Criteria dashboard and maturity descriptor authoring**
   ...
   - Descriptor text must be auditor-operable: each level statement starts from the actual accepted criterion evidence requirement, restated as an auditable subject, then defines the Basic/Reactive/Compliant/Proactive/Resilient evidence state for that exact requirement.
