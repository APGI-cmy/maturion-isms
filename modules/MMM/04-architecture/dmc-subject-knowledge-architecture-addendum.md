...
11. **Criteria dashboard and maturity descriptor authoring**
   ...
   - Descriptor text must be auditor-operable: each level statement starts with `Evidence that ...`, restates the actual accepted criterion evidence requirement as a grammatical auditable clause, then defines the Basic/Reactive/Compliant/Proactive/Resilient evidence state for that exact requirement.
