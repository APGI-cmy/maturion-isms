...
11. **Criteria dashboard and maturity descriptor authoring**
   ...
   - Descriptor text must preserve the criterion-specific actor/action/object anchor. Maturity methodology supplies the level posture, but the accepted criterion supplies the evidence subject; role, reporting-line, meeting, support, and escalation criteria must not collapse into generic policy/control wording.
   - Descriptor text must be auditor-operable: each level statement starts with a grammatical evidence lead-in, normally `Evidence that ...`, restates the actual accepted criterion evidence requirement as an auditable clause, then defines the Basic/Reactive/Compliant/Proactive/Resilient evidence state for that exact requirement.
   - Descriptor lead-ins must be self-corrected before display. Infinitive source wording such as `To indicate...` must become `Evidence indicating...`, conditional source wording such as `Where possible...` must become `Evidence that, where possible...`, and generated text must not emit malformed starts such as `Evidence that To...` or `Evidence that Where...`.
   - Descriptor evidence clauses must exclude explicit explanatory notes from verbatim criteria. Parenthetical or bracketed `Note:`, `Guidance:`, or `Reference:` text remains visible in the accepted criterion for user context, but descriptor generation treats it as context only and does not include it in Basic/Reactive/Compliant/Proactive/Resilient evidence statements. This exclusion applies to deterministic methodology drafts and to AI-refined descriptor text before display or save.
   - In Verbatim mode, multi-sentence accepted criteria must not be split during descriptor authoring. They should be compressed into one grammatical evidence clause so the verbatim requirement remains intact while the maturity descriptor remains readable and auditable.
   ...
   - Descriptor edit learning must be visible to the user. The first descriptor text edit for a criterion prompts Maturion to thank the user for the guidance and ask whether to record the edit in learning memory; accepting records changed levels as learning telemetry, while declining still saves the descriptor text without memory capture for that criterion.
   - Criterion codes are one-based sequence identifiers within each MPS. Generated and manually added criteria must be normalized to `<MPS>.C001`, `<MPS>.C002`, etc. (or the existing hyphenated MPS separator), saved with matching one-based `sort_order`, and displayed as a visible `Sequence` rather than exposing historical raw zero-based sort values.
