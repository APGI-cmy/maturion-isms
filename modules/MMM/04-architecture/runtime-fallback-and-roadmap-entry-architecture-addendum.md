...
- Organisation-context source ingestion must run a dedicated AI parse stage (`/api/v1/parse`) before chunk persistence for non-trivial PDF sources, then materialize canonical verbatim index rows (`mmm_org_source_verbatim_index`) keyed by `domain_name` + `mps_code`.
- In `VERBATIM` mode, runtime intent resolution must query `mmm_org_source_verbatim_index` first; if no indexed verbatim intent exists for the target MPS, generation must fail with explicit diagnostics (no silent generic downgrade).
- `source_mode:VERBATIM` documents must not be marked `completed` unless at least one canonical verbatim index row was generated; otherwise status is `failed` with parse-note guidance.
