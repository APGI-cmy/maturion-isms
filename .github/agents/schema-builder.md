...
> The first file you read in any session is THIS file — `.github/agents/schema-builder.md`.
...
8. **B_H**: Implement database schema to satisfy RED tests
...
- **Parking Station**: Append one-line summary per suggestion to `.agent-workspace/schema-builder/parking-station/suggestions-log.md` (create if absent). Format: `| YYYY-MM-DD | <agent> | <session-NNN> | <one-sentence summary> | <session-memory-filename> |`
