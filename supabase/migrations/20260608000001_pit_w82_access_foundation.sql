...
CREATE TABLE IF NOT EXISTS public.user_roles (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  CHECK (role = 'cs2_admin' OR org_id IS NOT NULL)
);

CREATE UNIQUE INDEX IF NOT EXISTS idx_user_roles_org_scope_unique
  ON public.user_roles (user_id, org_id, role)
  WHERE project_id IS NULL AND org_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS idx_user_roles_project_scope_unique
  ON public.user_roles (user_id, org_id, project_id, role)
  WHERE project_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS idx_user_roles_global_cs2_unique
  ON public.user_roles (user_id, role)
  WHERE role = 'cs2_admin' AND org_id IS NULL AND project_id IS NULL;
...
CREATE OR REPLACE FUNCTION public.pit_is_cs2_admin()
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
SET search_path = public, auth, pg_temp
AS $$
  SELECT public.pit_is_cs2_admin()
    OR EXISTS (
      SELECT 1
...
CREATE POLICY organisations_select_member
  ...
  FOR SELECT
  TO authenticated
  USING (public.pit_is_org_member(id));
...
CREATE POLICY user_roles_select_admin
  ...
  FOR SELECT
  TO authenticated
  USING (user_id = auth.uid() OR public.pit_has_org_role(org_id, ARRAY['cs2_admin','org_admin']) OR public.pit_is_cs2_admin());
...
CREATE POLICY user_roles_write_admin
  ...
  FOR ALL
  TO authenticated
  USING (public.pit_has_org_role(org_id, ARRAY['cs2_admin','org_admin']) OR public.pit_is_cs2_admin())
  WITH CHECK (public.pit_has_org_role(org_id, ARRAY['cs2_admin','org_admin']) OR public.pit_is_cs2_admin());
...
CREATE POLICY audit_log_insert_member
  ...
  FOR INSERT
  TO authenticated
  WITH CHECK (actor_id = auth.uid() AND public.pit_is_org_member(org_id));
...
CREATE POLICY qa_runs_write_cs2
  ...
  TO authenticated
  USING (public.pit_has_org_role(org_id, ARRAY['cs2_admin']))
  WITH CHECK (created_by = auth.uid() AND public.pit_has_org_role(org_id, ARRAY['cs2_admin']));
