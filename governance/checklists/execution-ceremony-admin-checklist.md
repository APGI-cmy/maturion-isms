...
| 13.3 | If any protected-path file is present in the diff, set `ecap_required: YES` in PREHANDOVER YAML | Check PREHANDOVER YAML field `ecap_required` — must be `YES` if any protected-path file appears in diff | | |
| 13.4 | Verify ECAP/admin ceremony was appointed and completed for this wave (`ceremony_admin_appointed` in `wave-current-tasks.md` reflects appointment; ECAP bundle artifacts are committed) | Check `wave-current-tasks.md` `ceremony_admin_appointed` field AND verify ECAP reconciliation summary exists and is committed; set `ecap_invoked: YES` in PREHANDOVER YAML | | |
