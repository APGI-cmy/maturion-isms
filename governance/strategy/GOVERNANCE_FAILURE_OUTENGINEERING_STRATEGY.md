...
|---|---|
| Single state evaluator | Controller, producer guidance, pre-handover, QP, ECAP, watchdog, and IAA injection use the same authoritative PR-scoped state evaluator. |
...
| Terminal guard | Block automatic assurance/build restarts after valid PASS for unchanged reviewed content and authority context. Permit explicit material findings or invalidated evidence to reopen only the affected lane. |
...
| FO-012 | Guidance asks for pre-brief after final PASS | Terminal-state defect | terminal guard plus post-PASS regression test | Controller | Under repair | hard gate |
...
| W3 — Controller state, merge and loop safety | shared evaluator; idempotency; delta/terminal controls; injections; scoped review/merge adapter and successor queue | W0; controller foundation; frozen W1/W2 interfaces; approved W4 active-CS2 authority | Test/design with W1/W2; one active pilot job; shared evaluator changes serialized | Foreman → appointed controller/QA builders | state/property tests; unsafe/stale/self-authorizing merge refused; duplicate/reordered events; simulated 24-hour repeat stopped without live spending |
